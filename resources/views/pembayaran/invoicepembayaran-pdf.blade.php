...
Dari:  {{ $data->nama }} <br>
Sebesar :{{ $data->total_bayar }} <br>
Pada Tanggal : {{ $data->updated_at->toDateString() }} <br>
Untuk Pembayaran Kamar Kost no  {{ $data->no_kamar }} <br>
Tipe kamar :{{ $data->tipe_kamar }} <br>
Jenis Sewa :{{ $data->sewa }} <br>
Periode: {{ $data->updated_at->toDateString() }} S/D  {{ $data->tanggal_bayar }} <br>


<br>
<br>
Bandung, {{ $date=$data->updated_at->toDateString() }}
