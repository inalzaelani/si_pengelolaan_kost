...
<div class="container">
  ...
  <div class="row justify-content-center">
    <div class="col-8">
      <div class="card">
        <div class="cardbody">
          <form action="/updatedatadiri/{{ Auth::guard('occupant')->user()->id }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="mb-3">
                <label for="exampleInputEmail1" class="form-label">Username</label>
                <input type="text" class="form-control" id="username" name="username" aria-describedby="emailHelp" value="{{ Auth::guard('occupant')->user()->username }}">
              </div>
              <div class="mb-3">
                <label for="exampleInputEmail1" class="form-label">password</label>
                <input type="password" class="form-control" id="password" name="password" aria-describedby="emailHelp" value="{{ Auth::guard('occupant')->user()->password }}">
                <input type="checkbox" onclick="showPassword()">Tujukan Password
              </div>
            <div class="mb-3">
              <label for="exampleInputEmail1" class="form-label">Nama</label>
              <input type="text" class="form-control" id="username" name="nama" aria-describedby="emailHelp" value="{{ Auth::guard('occupant')->user()->nama }}">
            </div>
            <div class="mb-3">
              <label for="exampleInputEmail1" class="form-label">Jenis Kelamin</label>
              <select class="form-select" name="jenis_kelamin" aria-label="Default select example">
                <option selected >{{ Auth::guard('occupant')->user()->jenis_kelamin }}</option>
            ...
            <div class="mb-3">
              ...
              <input type="number" class="form-control" id="username" name="no_telp" aria-describedby="emailHelp" value="0{{ Auth::guard('occupant')->user()->no_telp }}">
            </div>
